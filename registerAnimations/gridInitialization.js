  ...
  updateGridPoints(mouseX, mouseY) {
    ...
    this.gridPoints.horizontal.forEach((line) => {
      line.forEach((point, pointIndex) => {
        // Base wave effect
        const wavePhase =
          this.time + (pointIndex / line.length) * 2 * Math.PI * waveDensity;
        const baseWaveY = Math.sin(wavePhase) * waveAmplitude;
        const horizontalWave =
          Math.cos(this.time * 0.7 + pointIndex * 0.2) * (waveAmplitude * 0.2);
    ...
    this.gridPoints.vertical.forEach((line) => {
      line.forEach((point, pointIndex) => {
        ...
        const verticalWave =
          Math.sin(this.time * 0.7 + pointIndex * 0.2) * (waveAmplitude * 0.2);
